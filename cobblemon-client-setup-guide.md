...
지도/위키 편의 모드는 선택입니다. 없어도 서버 접속은 됩니다.
...
`Wiki Cobblemon`은 인게임에서 포켓몬 정보를 검색하는 클라이언트 위키입니다. 서버 접속에는 필수가 아니며, 개인 편의용으로만 사용합니다.
...
기술과 특성
기본 스탯과 EV
현재 위치에서 스폰 조건을 만족하는지 확인
```

기본 키가 기존 Cobblemon 키와 겹칠 수 있어서, 현재 로컬 테스트 프로필은 아래처럼 맞춰 두었습니다.

| 행동 | 키 |
| --- | --- |
| Wiki Cobblemon 열기 | `J` |
| 바라보는 포켓몬 위키 열기 | `K` 길게 누르기 |
| 팀 빌더 열기 | `;` |

키가 다르거나 충돌하면 Minecraft `Options` -> `Controls`에서 `TropiWiki` 항목을 검색해 바꿉니다. 포켓몬 상세 화면에서 `Check` 버튼을 누르면 현재 위치 기준으로 시간, 날씨, 높이, 바이옴 같은 스폰 조건을 만족하는지 확인할 수 있습니다.
